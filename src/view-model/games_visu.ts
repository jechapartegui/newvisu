  ...
  constructor(_match: full_game, _player: player_game[], _off: officials[], _ref: referee[], _score: detailed_score[], _event: game_events[]) {
    if (_match) {
      ...
      this.match.events.forEach(ev => {
        if (ev.playerid1 > 0) {
          let player1 = _player.filter(x => x.id == ev.playerid1);
          ev.player1photo = player1.length > 0 ? player1[0].photo : null;
        }

        if (ev.playerid2 > 0) {
          let player2 = _player.filter(x => x.id == ev.playerid2);
          ev.player2photo = player2.length > 0 ? player2[0].photo : null;
        }
